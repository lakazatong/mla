...
#define START_CHRONO CONCAT(start, __LINE__) = std::chrono::high_resolution_clock::now();
#define TOP_CHRONO CONCAT(top, __LINE__) = std::chrono::high_resolution_clock::now(); std::chrono::duration<double, std::milli> CONCAT(elapsed, __LINE__) = CONCAT(top, __LINE__) - CONCAT(start, __LINE__); CONCAT(total, __LINE__) += CONCAT(elapsed, __LINE__).count(); CONCAT(hits, __LINE__)++; CONCAT(start, __LINE__) = CONCAT(top, __LINE__);
